...
- 通过官方 `sidebar.footer.action` 注册“即我”入口；打开后在 Footer 内联展开即我目录，并以 `priority: -10` 临时替换官方 `conversation` 中间栏。关闭、点击“新会话”或切换原生 Session 时下拉列表收起，原生 Conversation 自动恢复；
...
- 打开即我后默认进入“发给自己”并选中默认分类；返回会话列表后按桌面端样式展示头像、群头像拼图、名称、摘要、时间、未读和选中态。
