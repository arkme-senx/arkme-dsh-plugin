...
describe('ArkmeSettingsSurface', () => {
  it('keeps Arkme account and APP update capabilities with a read-only plugin version', () => {
    ...
    expect(source).toContain("unbind ? 'auth.phone.unbind.send' : 'auth.phone.send'")
    expect(source).toContain("unbind ? 'auth.phone.unbind' : 'auth.phone.verify'")
    ...
    expect(source).not.toContain('title="位置记录"')
    expect(source).not.toContain('LocationPermissionDialog')
