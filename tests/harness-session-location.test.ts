...
it('uses runtime/session identity, hides same-computer instances, and follows rename/offline without another poller', async () => {
  ...
  // Finish discovery before mounting: an in-flight pull would hide a redundant refresh.
  await act(async () => {})
  expect(callArkme).toHaveBeenCalledTimes(1)
  root = createRoot(container)
  const render = async (sessionId: string) => { await act(async () => root!.render(createElement(SessionComputerLocation, { key: sessionId, surface, sessionId, t }))) }
...
it('withdraws old computer information on account/environment changes and ignores the old pending response', async () => {
  ...
  const install = () => installHarnessAccountSessions({ slots: { entries: () => [], subscribe: () => () => {} } } as never, surface)
  stop = install()
  root = createRoot(container)
  ...
  await act(async () => {
    stop!()
    surface.dataset.arkmeAccountId = '2'; surface.dataset.arkmeAccountScope = 'prod'
    stop = install()
  })
  expect(container.textContent).toBe('')
