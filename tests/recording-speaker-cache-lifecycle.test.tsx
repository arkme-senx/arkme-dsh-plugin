...
const item: ArkmeRecordingWorkbenchItem = {
  ...
  speakerNumber: 1, speakerKey: 'transcript-speaker', canBindSpeaker: false, transcriptSource: 'system', sessionKey: 'session', textStartOffset: 0, textEndOffset: 2, textTotalLength: 2,
...
describe('speaker cache interaction and lifecycle', () => {
  ...
  it.each([
    ...
    await act(async () => {
      renderer = create(<ArkmeRecordingSpeakerEditor item={{ ...item, canBindSpeaker: true }} forceBatchUpdate={scenario.force} onUpdated={onUpdated} onClose={onClose} />)
      await tick()
    })
    await chooseUser()
