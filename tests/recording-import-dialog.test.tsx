...
describe('recording import dialog behavior', () => {
  ...
  it('deduplicates equal files inside one browser selection', async () => {
    const first = new File(['same'], '会议.m4a', { type: 'audio/mp4' })
    const second = new File(['same'], '会议.m4a', { type: 'audio/mp4' })
  ...
  it('renders background tasks inside the same seven-column desktop table', async () => {
    mocks.callArkme.mockImplementation(async operation => {
      if (operation === 'recordings.import.list') return [{
        ...
        durationMillis: 3_000, progress: .5, createdAtMillis: 1, updatedAtMillis: 2,
      }]
      throw new Error(`unexpected operation: ${String(operation)}`)
    })
    await act(async () => {
  ...
  it('lets the user stop a pending retry or cancel command instead of trapping the modal', async () => {
    let mutationSignal: AbortSignal | undefined
    mocks.callArkme.mockImplementation(async (operation, _params, signal?: AbortSignal) => {
      if (operation === 'recordings.import.list') return [{
        importRef: 'opaque', revision: 2, phase: 'failed', fileName: '失败.m4a', fileSize: 2_048,
        durationMillis: 3_000, progress: .5, createdAtMillis: 1, updatedAtMillis: 2, retryable: true,
      }]
      if (operation === 'recordings.import.retry') {
    ...
    await act(async () => { renderer.root.findByProps({ 'aria-label': '重试 失败.m4a' }).props.onClick(); await tick() })
    await act(async () => { button(renderer, '停止当前操作').props.onClick(); await tick() })
