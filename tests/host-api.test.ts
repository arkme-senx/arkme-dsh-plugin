...
import { dispatchJotmoHostOperation } from '../src/host-api.js'
...
describe('outgoing call Host API dispatch', () => {
  it('rejects an unknown outgoing media type before calling the service', async () => {
    ...
    await expect(dispatchJotmoHostOperation(service as never, 'calls.outgoing.prepare', {
  ...
  it('passes only the strict prepare fields to the service', async () => {
    ...
    await dispatchJotmoHostOperation(service as never, 'calls.outgoing.prepare', {
  ...
  it('requires non-empty one-time intent credentials', async () => {
    ...
    await expect(dispatchJotmoHostOperation(service as never, 'calls.outgoing.intent.resolve', {
  ...
  it('accepts calling completion without forwarding caller-supplied failure text', async () => {
    ...
    await dispatchJotmoHostOperation(service as never, 'calls.outgoing.intent.resolve', {
  ...
  it('validates heartbeat and release request IDs', async () => {
    ...
    await expect(dispatchJotmoHostOperation(service as never, 'calls.outgoing.heartbeat', {
      callRequestId: '',
    })).rejects.toMatchObject({ code: 'call-request-invalid' })
    await dispatchJotmoHostOperation(service as never, 'calls.outgoing.release', {
