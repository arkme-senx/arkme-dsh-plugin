...
it('keeps a failed write reviewable and never silently retries with a newer revision', async () => {
  ...
  expect(host.querySelector('[role=dialog]')?.textContent).toContain('ARCHIVE_REVISION_CONFLICT')
  expect(host.querySelector('[role=dialog]')?.textContent).not.toContain('确认归档')
})

it('keeps the in-flight confirmation alive when a record notification beats its reply', async () => {
...
it('serializes paginated loading and ancestor lookup without leaving a stuck loading state', async () => {
  ...
  await click('加载更多')
  const origin = [...host.querySelectorAll('button')].find(node => node.textContent === '查看归档来源')!
  ...
  await click('查看归档来源')
  expect(origin.disabled).toBe(true)
  expect(host.querySelector('[role=status]')?.textContent).toBe('加载中…')
...
it('keeps menu confirmation and captured revision after its archived directory row disappears', async () => {
