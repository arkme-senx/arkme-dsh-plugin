...
describe('Arkme tool catalog', () => {
  it('keeps the current business tool surface and stable order', () => {
    expect(arkmeToolCatalog.toolNamesFor('business')).toEqual([
      ...
      'arkme_source_members_page',
      'arkme_source_members_presentation',
      'arkme_source_members_cached',
  ...
  it('separates registration phase from business kind and write policy', () => {
    ...
    expect(writes.map(module => module.meta.toolName)).toEqual([
      ...
      'arkme_world_publish_text', 'arkme_extension_review_create', 'arkme_recording_import', 'arkme_recording_import_folder', 'arkme_bot_conversation_pin', 'arkme_topic_home_visibility', 'arkme_archive_set', 'arkme_group_member_add', 'arkme_group_member_remove', 'arkme_group_join_restriction_set', 'arkme_group_self_nickname_set', 'arkme_copy_link_extend', 'arkme_conversation_mark_read', 'arkme_message_report', 'arkme_message_withdraw', 'arkme_user_ban', 'arkme_user_unban', 'arkme_direct_message_refusal_set', 'arkme_group_ai_polish_manage', 'arkme_reactions_write',
