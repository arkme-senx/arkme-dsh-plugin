...
import {
  ...
  recordingTimelineVisibleSpeakerCount,
...
describe('recording timeline math', () => {
  ...
  it('fits the same number of fixed-width speaker entries as desktop', () => {
    expect(recordingTimelineVisibleSpeakerCount(696, 19)).toBe(6)
    expect(recordingTimelineVisibleSpeakerCount(502, 19)).toBe(4)
    expect(recordingTimelineVisibleSpeakerCount(696, 3)).toBe(3)
  })
  ...
  it('keeps the complete desktop timeline and import entry on an empty day', () => {
    ...
    expect(markup).not.toContain('全天说话人图例')
  })
  ...
  it('keeps the full-day speaker legend stable when the default viewport excludes a speaker', () => {
    const dayStart = new Date(2026, 7, 28).getTime()
    const items = [
