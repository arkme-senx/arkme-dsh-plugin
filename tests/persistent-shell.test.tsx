...
describe('Arkme persistent DSH shell', () => {
  ...
  it('renders an Arkme-owned sidebar rail for the lifetime of the plugin', () => {
    ...
    expect(markup).toContain('data-arkme-qq2006-directory-chrome="true"')
    expect(markup).toContain('DeepSeek Harness')
    expect(markup).toContain('在线 · Arkme')
    expect(markup).toContain('aria-label="搜索对话或消息"')
    expect(markup).toContain('aria-label="刷新会话"')
