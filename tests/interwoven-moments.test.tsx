...
describe('interwoven UI', () => {
  ...
  it('renders distinct loading, error, success and degraded detail states', () => {
    ...
    for (const markup of [loading, error, success, degraded, degradedSummary]) {
      ...
      expect(markup).toContain('width:min(405px, 100%)')
      expect(markup).toContain('top:56px')
      ...
      expect(markup).toContain('var(--dsw-alias-bg-base')
      expect(markup).toContain('var(--dsw-alias-border-l2')
    }
  })
})
