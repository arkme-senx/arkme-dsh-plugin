...
describe('ArkmeSettingsSurface', () => {
  ...
  it('renders only functional account settings in the plugin surface', () => {
    ...
    expect(aboutMarkup).toContain('class="arkme-about-product"')
    expect(aboutMarkup).toContain('邀请得会员')
    ...
    expect(aboutMarkup).not.toContain('<h2>更新</h2>')
  ...
  it('offers logout only for an authenticated account', () => {
    ...
    try {
      ...
      expect(authenticated).not.toContain('账户余额')
      expect(authenticated).not.toContain('>充值<')
