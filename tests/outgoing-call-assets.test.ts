...
async function request(path: string, method = 'GET') {
  ...
  await createOutgoingCallAssetHandler({
    routePrefix: '/jotmo-self/api/call',
    assetDirectory,
  })(req, res)
...
describe('outgoing call assets', () => {
  it('serves the iframe document with the script policy required by the pinned CallEngine bundle', async () => {
    const response = await request('/jotmo-self/api/call/index.html')
  ...
  it('serves the bundle, icon, and manifest with exact content types', async () => {
    const bundle = await request('/jotmo-self/api/call/bundle.js')
    const icon = await request('/jotmo-self/api/call/call-linear-strong.svg')
    const manifest = await request('/jotmo-self/api/call/manifest.json')
  ...
  it('supports HEAD and rejects methods, unknown files, and traversal', async () => {
    const head = await request('/jotmo-self/api/call/bundle.js', 'HEAD')
    expect(head.status).toBe(200)
    expect(head.body).toHaveLength(0)
    expect((await request('/jotmo-self/api/call/bundle.js', 'POST')).status).toBe(405)
    expect((await request('/jotmo-self/api/call')).status).toBe(404)
    expect((await request('/jotmo-self/api/call/unknown.js')).status).toBe(404)
    expect((await request('/jotmo-self/api/call/../package.json')).status).toBe(404)
    expect((await request('/jotmo-self/api/call/%2e%2e%2fpackage.json')).status).toBe(404)
  })
