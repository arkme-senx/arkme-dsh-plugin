...
it('reuses the original row and only opens the existing topic directory',async()=>{
  ...
  // Count the rendered scroll owner, not both the component and its DOM node.
  expect(renderer!.root.findAll(node=>typeof node.type==='string' && node.props['aria-label']==='发给自己分类')).toHaveLength(1)
  await act(async()=>renderer!.root.findByProps({'aria-label':'返回 Arkme 会话列表'}).props.onClick())
  expect(renderer!.root.findAll(node=>typeof node.type==='string' && node.props['aria-label']==='发给自己分类')).toHaveLength(0)
