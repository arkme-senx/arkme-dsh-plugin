...
git -C "$ARKME_DSH_CHECKOUT" diff --quiet "${ARKME_DSH_REF:-origin/master}" HEAD --
...
run_topic_archive_cross_test() (
  ...
  pnpm exec vitest run --config "$task_plugin_root/vitest.topic-archive-e2e.config.mts"
)

e2e_run_compose_go_test "topic-archive-cross" \
  "topic archive: packed DSH UI + SDK + Tool + real Record owner" run_topic_archive_cross_test
