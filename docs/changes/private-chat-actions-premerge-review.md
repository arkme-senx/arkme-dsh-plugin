...
最终基线复核先后发现 `dev` 新增 `952d83e` 图片预览更新和 `3acb791` 快记背景音更新；均已在任务分支通过 merge 纳入，未覆盖已推送历史，也未合入 `dev`。两次更新均无任务代码冲突；最后一次合入后重新执行全量测试与制品构建，460 个文件通过、4 个跳过；5,124 项测试通过、6 项跳过。下文保留基线推进前的分轮记录。

合入后另外重新打包、安装并启动隔离官方 DSH，页面冷缓存/30 秒慢请求仍直接显示“拒收对方消息”“封禁用户”，无查询加载占位。安装后的 `lib/client.js` 与本次构建 SHA-256 均为 `f26537ad27db96eecad290f1fdbfec74049cb6ba5bf8868b637df8fd74550298`；该不可变 `.tgz` SHA-256 为 `0d4f53575336247f912a899ec8d9033ec0decdceb9ad8710652161bd9f2a900b`。验收页与本任务临时服务已关闭，未操作用户常驻实例。
