...
| Tools | 既有 arkme_records_search 共用 SearchService，返回校验后的关联和跳转目标；真实会话调用待验收 |
| SDK | searchRemote 公开远端搜索及能力探测；合同测试通过，独立消费者运行待验收 |
...
按用户最终选择，右栏同步记录点击与左栏双击/Enter 均优先打开本地对应 DSH 对话；本机不存在或历史关联未知时保留同步消息兜底。右栏明确提示当前公开接口不支持精确定位消息，未使用私有 DOM/消息组件接口。
