...
- 正式 DSH 模型会话验收已尝试：在最终插件制品对应的专用 Windows 测试客户端中新建只读验收会话，要求读取测试账号的短语库。官方运行时在模型请求阶段返回 `MISSING_CREDENTIAL`（deepseek-official），未执行工具调用。因此合并请求保持草稿，不能将注册/合同测试或这次失败尝试等同为模型会话验收通过；需在测试客户端配置可用模型后继续验证。
...
Chat 相关测试通过；World 权限/发布状态测试通过。World 按现有配置加载约束编译后从仓库根目录运行，裸包目录执行的配置缺失与实际断言结果分开记录。新增 Record 修复已推送正式合并分支，尚未部署测试或生产环境，不应把此前测试服构建号视为包含此修复。
