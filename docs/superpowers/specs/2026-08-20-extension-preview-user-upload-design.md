...
> 后续合同说明：本文件以下正文保留初版“仅用户附件”设计记录。当前实现已扩展为三种受控来源：Arkme `image_ref`、准备阶段捕获的最新直接用户消息附件，以及当前 Agent workspace 内的唯一相对路径。头像和预览新增均改为 Host 强制的 `prepare → 后续直接用户精确回复 → confirm` 对话确认；确认阶段重读并核对目标与内容指纹，不再使用 ACK 卡。任意绝对路径、遍历、符号链接逃逸、URL 和未绑定来源仍然禁止。
