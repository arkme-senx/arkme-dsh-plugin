...
CI 检查插件内资产与记录的 hash 一致，避免未记录的人工修改。为严格落实本期“不接听”，随包 `bundle.js` 在上游产物基础上只增加 `outgoingOnly` 来电监听守卫；清单同时记录上游原始 hash 与派生产物 hash。视觉 DOM、设备交互与呼叫状态机保持上游实现。后续升级通过显式同步完成，不在插件构建时跨仓库隐式读取。
