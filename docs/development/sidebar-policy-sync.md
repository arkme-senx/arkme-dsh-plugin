...
- 最终 `NODE_OPTIONS=--no-experimental-webstorage pnpm test --maxWorkers=4`：687 个测试文件通过、8 个跳过；8284 项通过、12 项跳过。相同参数下原始基线为 8262 项通过、12 项跳过，本次新增 22 项。
...
- `pnpm build` 与 `pnpm pack` 通过；不可变 tgz SHA-256：`33f3dd5a2aae5064c7f2966fb50eead38fc6706d0f43cabf25b9ec4fa093dfa3`。包内未发现本机路径、测试目录、数据库或临时日志；版本保持 0.1.69，未发布。
