...
日期：2026-09-09。Audio 与 OpenAPI 已分别 squash 合入 master（4e18ff7、16e3c8e）；插件、Intelligent 与 Meta 尚未合并。当前按用户确认的范围收口：服务端仅完善开放平台只读能力，不改视频业务合同；Audio 从最新 master 新建修正分支，插件保留必要的客户端输入适配，Intelligent 本次差异撤除。最新验收与发布顺序见 Meta 同任务 change 的 scope-cleanup.md；premerge-review.md 和 verification.md 保留此前验收历史。
