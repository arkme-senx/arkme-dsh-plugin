...
日期：2026-09-09。Audio 与 OpenAPI 首轮能力已分别 squash 合入 master（4e18ff7、16e3c8e）；Audio 范围收口修正、插件与 Meta 仍需合并。服务端仅完善开放平台只读能力，不改视频业务合同；Audio 修正使用基于最新 master 的新分支，插件复用既有通用 MCP 并保留必要的消费者输入适配。Intelligent 本次差异已撤除，不作为合并或发布依赖。插件已集成 dev 9d293a6，最新责任边界及验收见 Meta 同任务 change 的 mcp-boundary-cleanup.md；发布顺序仍为服务端先验收，再发布客户端。

合并前补充：dev 更新至 92605ab 后新增的录音文件/目录导入工具属于独立的本地文件获取能力，予以保留；recordings module 与 port 仅移除旧 days/read 及其专属方法，不整目录退役。导入完成不是转写就绪，import_ref 不是 recording_uid；新导入描述也不得引导调用已退役的读取工具。
...
确定开发量：复用 3 个现有能力、扩展 1 个现有能力、新增 2 个原子读取能力，复用插件既有通用 MCP 接入，完成旧入口退役及必要的既有消费者输入适配、文档和测试。不新增报告生成工具。
...
- 录音 UID、总结 UID 是业务标识；数值用户身份继续现有 user_ref；公开游标继续现有 Actor/Capability/结果集认证，不发明新 Ref 框架。
...
- 从 src/tools/business/index.ts 的目录组合及全部 Profile 移除两个旧录音模块，删除其模型执行定义及旧调用指引。共享业务 service 是否保留按真实调用者裁决，不能连带误删。
- 保留现有 ManagedMcpController 的账号/设备绑定、凭据管理、执行隔离和生命周期，不另建录音专用 Key 或手写 MCP 协议。
- 工具名称、描述、输入及输出 Schema 由开放平台 tools/list 经官方 MCP Client 通用发现和注册；插件不维护录音能力清单、参数探测或专属 Prompt，也不复制开放平台工具组合规则。新增平台工具无需插件追加注册逻辑。
- 调用语义由开放平台工具合同与配套文档统一维护；Agent 运行时依据实际下发的工具 Schema，而非假定加载了平台 Markdown 文档。缺少某个工具不影响其他已发现工具，不创建旧后端回退。
- 既有插件消费工具只描述自身的输入与授权要求；其必要输入适配不参与 MCP 发现，不接管开放平台读取职责。
...
2. 插件 Host 复用已有 APP 录音详情接口取得素材定位，匹配用户选定的完整原句，不让模型猜测分片 ID、ASR 索引或来源。权限与素材读取仍由现有服务端裁决，不新增 APP 解析接口。
3. 不把 full 模式的 utterance_index 当旧 ASR 数组索引；不将无法证明相同的规范话语强行映射为旧来源，也不静默扩大选区。
4. 输入保留 recording_uid + 完整原句 text/start_offset_ms/end_offset_ms。Host 调用既有 get-session-detail-by-id，以详情的绝对 start_at/end_at 减去 session.start_at，与原句时间和全文精确匹配；详情的 start_offset_millis/end_offset_millis 是分片内偏移，不能直接当录音偏移。仅使用响应中真实 asr_item_index，不使用列表序号。定位结果只在 Host 内部使用。
5. 这项适配与旧入口删除同批完成；不修改 Intelligent 视频模型、预检、任务复用或持久化，不治理录音与视频已有的内部定位耦合。
6. 依赖场景未验证通过时，本次替代不允许发布；不能恢复旧工具作为验收兜底，也不能未经确认直接砍掉其他现有业务能力。

原句不存在、时间/正文不符或定位歧义时，客户端在视频预检前拒绝。后续消费沿用服务端现有预检、素材快照及幂等语义；本次不新增跨读取时刻的内容指纹条件，也不宣称解决历史定位与消费之间的全部并发问题。同一分片两种来源的全文及时间完全一致时优先使用增强来源；跨分片或同来源重复匹配不能猜测。
...
- 实现范围为 Audio 的开放平台只读支撑、OpenAPI、插件与 Meta。Intelligent 不作为本次合并或部署依赖，不进行视频领域迁移。
...
- 先部署包含范围收口修正的 Audio，再部署 OpenAPI；确认旧客户端 APP 调用、既有视频请求、REST/MCP 目录及真实读取通过后，才发布并重载插件。Intelligent 无本次部署要求。服务端先上线期间不封禁旧客户端共享接口，新客户端不回退到旧工具。
...
- Audio/OpenAPI 已合入主干不等于已经部署；本轮只落地范围修正并验收，不执行生产部署或客户端发布。此前聊天页面 9 次真实读调用是历史证据，不能替代修正后的复验。
