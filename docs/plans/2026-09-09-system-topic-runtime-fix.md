...
以普通 merge 合入 `dev@1fdae5e`，并在验证期间基线再次推进后同步 `dev@c1d0c0d`；它们已经包含 `master@537a8b0` 的发布内容。原有开发 PR 目标仍为 dev，本次不调整 PR 目标、不发布。包版本 0.1.50 来自合入的上游发布提交；任务相对 dev 不改版本、根 README 或 lockfile。
...
后续基线的多选修复也完整保留：普通聊天编辑器在多选时保持挂载、保留草稿与视口；系统主题不挂载新增编辑器，而由设置栏提供同一稳定底部 slot，多选时设置隐藏且 inert。真实 Chrome 验证系统主题进入/退出多选高度不变、没有新增编辑器、退出后开关恢复可用。
...
最终包 SHA-256：`e8aea8b9131c4a3b33e802913a18d86da54cd3973ca8ae6c73d49ee9db785e76`。不将开发构建目录作为产物校验锚点：部分全量测试会重新 bundle，验收以压缩包和实际安装内容为准。

最终复跑结果：487 个测试文件通过、6 个按现有规则跳过；5640 个测试通过、8 个跳过。TypeScript typecheck、构建/打包、正式包跨仓 E2E（含新增的系统主题多选交互）均通过。审查中新增的取消测试曾失败，修复及既有断言的可选 signal 参数对齐后，全量再次通过；没有通过删除业务断言或新增 skip 消除失败。
