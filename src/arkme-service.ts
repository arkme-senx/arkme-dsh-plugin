...
interface PendingChatNotificationHint {
  hint: ArkmeChatReceiveHint
  connectionGeneration: number
  attempts: number
}

interface PendingChatProjection {
...
export class ArkmeService {
  ...
  private handleChatRealtimeNotice(notice: ArkmeChatRealtimeNotice): void {
    if (notice.cause === 'reconcile') {
      ...
      this.emitChatClientEvent({
        ...
        connectionGeneration: generation, refresh: 'none',
      })
    ...
    try {
      ...
      for (let pageIndex = 0; pageIndex < 10; pageIndex += 1) {
        ...
        for (const item of page.items) {
          ...
          const source = await this.openSourceRef(item.sourceRef, session.userId)
  ...
  private scheduleChatSessionProjection(
    chatSessionUid: string,
    latestSequence: number,
  ...
  async resolveSource(sourceRef: string): Promise<ArkmeSourceItem> {
    const session = await this.requireSession()
    const source = await this.openSourceRef(sourceRef, session.userId)
    if (source.kind !== 'private_chat' && source.kind !== 'group_chat') {
      throw new ArkmePluginError('source-resolve-unsupported', '当前数据源不支持消息通知跳转', false)
    }
    const data = await this.authenticatedChatPost<Record<string, unknown>>(
      '/api/v1/chats/display-snapshots',
      { chat_session_uids: [source.ownerRef] },
      session,
    )
    const bundle = listValue(data.items)
      .map(item => objectValue(item))
      .find(item => stringValue(objectValue(item.session).chat_session_uid).trim() === source.ownerRef)
    if (bundle === undefined) {
      throw new ArkmePluginError('source-resolve-failed', '无法恢复消息对应的会话', true, 404)
    }
    const cacheKey = `${String(session.userId)}:${source.ownerRef}`
    const resolved = await this.chatSourceFromBundle(bundle, session, this.chatSourceCache.get(cacheKey), [])
    this.chatSourceCache.set(cacheKey, resolved)
    return resolved
  }
