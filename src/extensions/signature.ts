...
import {
  ARKME_BUNDLE_ARTIFACT_KIND, ARKME_BUNDLE_CONTRACT_VERSION,
  ARKME_NATIVE_BUNDLE_ARTIFACT_KIND, ARKME_NATIVE_BUNDLE_CONTRACT_VERSION,
  type ArkmeBundleExecutionModel,
} from './bundle-artifact.js'
import { ARKME_EXTENSION_FORMAT_VERSION, type ArkmeNativeCapability } from './types.js'
...
export function canonicalNativeBundleSignatureMessage(input: {
  artifact_contract_version: typeof ARKME_NATIVE_BUNDLE_CONTRACT_VERSION
  artifact_kind: typeof ARKME_NATIVE_BUNDLE_ARTIFACT_KIND
  extension_id: string
  package_name: string
  version: string
  ...
  return Buffer.from(JSON.stringify({
    ...
    package_json_sha256: input.package_json_sha256,
    source_sha256: input.source_sha256,
    // Go's canonical envelope uses `omitempty`: an empty capability set is
    // represented by the absent field, while non-empty values remain signed.
...
export function verifyNativeBundleResolutionSignature(
  resolution: {
    artifact_contract_version: typeof ARKME_NATIVE_BUNDLE_CONTRACT_VERSION
    artifact_kind: typeof ARKME_NATIVE_BUNDLE_ARTIFACT_KIND
    extension_id: string
  ...
  if (resolution.artifact_contract_version !== ARKME_NATIVE_BUNDLE_CONTRACT_VERSION
    || resolution.artifact_kind !== ARKME_NATIVE_BUNDLE_ARTIFACT_KIND
    || resolution.package_name.trim() === '' || resolution.execution_model !== 'dsh-native'
    || ![resolution.bundle_sha256, resolution.package_json_sha256, resolution.source_sha256]
      .every(value => /^[a-f0-9]{64}$/.test(value))
    || !Array.isArray(capabilities) || capabilities.some(value => typeof value !== 'string' || value.trim() === '')
    || capabilities.some((value, index) => index > 0 && capabilities[index - 1]! >= value)
    || !Number.isSafeInteger(resolution.published_at) || resolution.published_at <= 0
    || resolution.signature.trim() === '') {
    throw new ArkmePluginError('extension-signature-envelope-invalid', '原生 DSH V3 签名 envelope 字段无效', false, 502)
  }
  const key = trustedKeys.get(resolution.signing_key_id)
  if (key === undefined) {
    throw new ArkmePluginError('extension-signing-key-untrusted', `不信任扩展签名密钥 ${resolution.signing_key_id}`, false, 403)
  ...
  try {
    valid = verify(
      null,
      canonicalNativeBundleSignatureMessage(resolution),
      publicKey(key),
      Buffer.from(resolution.signature, 'base64'),
    )
  } catch (error) {
