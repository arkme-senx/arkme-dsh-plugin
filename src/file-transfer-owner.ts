...
export function createArkmeFileTransfers(options: {
  ...
  return new FileTransfers(options.directory, {
    ...
    send: async (input, assets, backgroundSound, expectedUserId, signal) => {
      ...
      const result = await options.chat.sendSourceRich(input.sourceRef, {
        ...input.content,
      ...
      try {
        ...
        return { ...result, warningText: `消息已发送，但位置快照未写入：${message}` }
      }
    },
