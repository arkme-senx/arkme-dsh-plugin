...
const BUSINESS_PROMPT_SUFFIX =
  ...
  + ' For group message withdrawal, member removal and join restrictions, use the current Arkme OpenAPI MCP tools. Resolve the group with resolve_chat_conversations, then use list_chat_members for user_ref and batch_get_group_members for version. For removed people, list_group_join_restrictions returns current names and user_ref; never guess from ambiguous or unavailable names. Existing message coordinates can go directly to withdraw_group_messages. Do not translate local source_ref, member_ref or message_withdrawal_ref into OpenAPI references. If MCP tools are unavailable, use the managed connection status and retry tools only when present and retryable; do not invent tools or claim success. Follow the existing conversational confirmation before executing governance writes.'
