...
import {
  JotmoOutgoingCallError,
  type JotmoOutgoingCallIntentClaim,
  type JotmoOutgoingCallIntentResolutionInput,
  type JotmoOutgoingCallMediaType,
  type JotmoOutgoingCallToolResult,
...
function nonEmpty(value: string, label: string): string {
  ...
  if (normalized === '') throw new JotmoOutgoingCallError('call-cancelled', `${label}不能为空`)
  return normalized
}

function validUserId(userId: number): number {
  if (!Number.isSafeInteger(userId) || userId <= 0) {
    throw new JotmoOutgoingCallError('call-cancelled', '呼叫账号无效')
  }
  return userId
}
...
export class JotmoOutgoingCallBroker {
  ...
  request(input: {
    ...
    if (this.disposed) return Promise.reject(new JotmoOutgoingCallError('call-cancelled', '呼叫服务已停止'))
    ...
    if (input.mediaType !== 'audio' && input.mediaType !== 'video') {
      return Promise.reject(new JotmoOutgoingCallError('call-cancelled', '呼叫媒体类型无效'))
    }
    if (input.signal?.aborted === true) {
      return Promise.reject(new JotmoOutgoingCallError('call-cancelled', '呼叫请求已取消'))
    }
    const intentId = this.randomId()
    const callRequestId = this.randomId()
    const expiresAtMillis = this.now() + INTENT_TTL_MS
    return new Promise<JotmoOutgoingCallToolResult>((resolve, reject) => {
      ...
      intent.timer = this.setTimer(() => {
        ...
        if (!current.toolSettled) {
          ...
          current.reject(new JotmoOutgoingCallError('call-ui-unavailable', '呼叫界面不可用，请打开 Harness 页面后重试'))
        }
      }, INTENT_TTL_MS)
      if (input.signal !== undefined) {
        const abort = (): void => {
          ...
          current.toolSettled = true
          current.reject(new JotmoOutgoingCallError('call-cancelled', '呼叫请求已取消'))
  ...
  resolveIntent(input: JotmoOutgoingCallIntentResolutionInput): void {
    ...
    if (intent === undefined) throw new JotmoOutgoingCallError('call-cancelled', '呼叫意图已失效')
    if (intent.userId !== input.userId) throw new JotmoOutgoingCallError('call-cancelled', '呼叫意图与当前账号不匹配')
    if (intent.claimToken === undefined || intent.claimToken !== input.claimToken) {
      throw new JotmoOutgoingCallError('call-cancelled', '呼叫意图领取凭据无效')
    }
    ...
    intent.reject(new JotmoOutgoingCallError(input.outcome.code, input.outcome.message))
  }
  ...
  acquireLease(userId: number, callRequestId: string): number {
    ...
    if (current !== undefined && current.callRequestId !== requestId) {
      throw new JotmoOutgoingCallError('call-active', '当前已有通话进行中')
    }
  ...
  heartbeatLease(userId: number, callRequestId: string): number {
    ...
    const current = this.leases.get(userId)
    if (current === undefined || current.callRequestId !== requestId) {
      throw new JotmoOutgoingCallError('call-cancelled', '呼叫租约已失效')
    }
  ...
  clearUser(userId: number, message: string): void {
    ...
    for (const intent of [...this.intents.values()]) {
      ...
      if (!intent.toolSettled) {
        intent.toolSettled = true
        intent.reject(new JotmoOutgoingCallError('call-cancelled', message))
      }
    }
  ...
  dispose(): void {
    ...
    for (const intent of [...this.intents.values()]) {
      ...
      if (!intent.toolSettled) {
        intent.toolSettled = true
        intent.reject(new JotmoOutgoingCallError('call-cancelled', '呼叫服务已停止'))
      }
    }
    this.leases.clear()
  }
  ...
  private expireStaleIntents(): void {
    for (const intent of [...this.intents.values()]) {
      ...
      if (!intent.toolSettled) {
        intent.toolSettled = true
        intent.reject(new JotmoOutgoingCallError('call-ui-unavailable', '呼叫界面不可用，请打开 Harness 页面后重试'))
      }
    }
  }
