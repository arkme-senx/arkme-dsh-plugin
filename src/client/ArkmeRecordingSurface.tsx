...
import { ArkmeRecordingTimeline, recordingTimelineSpeakerIdentity } from './recordings/ArkmeRecordingTimeline.js'
...
const styles: Record<string, CSSProperties> = {
  ...
  transcript: { minWidth: 0, padding: '13px 12px', display: 'grid', gridTemplateColumns: '32px minmax(0,1fr)', alignItems: 'start', gap: 11, borderRadius: 12, transition: 'opacity 120ms ease, background 120ms ease', contentVisibility: 'auto', containIntrinsicSize: '80px' },
  transcriptAvatar: { width: 32, height: 32, display: 'grid', placeItems: 'center', borderRadius: '50%', color: colors.base, fontSize: 11, fontWeight: 650 },
  transcriptBody: { minWidth: 0, position: 'relative' },
  ...
  speakerAvatarTrigger: { width: 32, height: 32, display: 'grid', placeItems: 'center', border: 0, borderRadius: '50%', padding: 0, background: 'transparent', color: 'inherit', cursor: 'pointer', font: 'inherit' },
  speakerNameTrigger: { display: 'inline-flex', alignItems: 'center', border: 0, padding: 0, background: 'transparent', color: 'inherit', cursor: 'pointer', font: 'inherit' },
...
export function recordingFocusedItems(
  items: readonly ArkmeRecordingWorkbenchItem[],
  focusedSpeakerKey?: string,
): { focusedSpeakerKey?: string; items: ArkmeRecordingWorkbenchItem[] } {
  if (focusedSpeakerKey === undefined) return { items: [...items] }
  const exists = items.some(item => recordingTimelineSpeakerIdentity(item) === focusedSpeakerKey)
  return exists ? { focusedSpeakerKey, items: [...items] } : { items: [...items] }
}
...
export function ArkmeRecordingSurface() {
  ...
  const [editingSpeaker, setEditingSpeaker] = useState<{ item: ArkmeRecordingWorkbenchItem; anchor: RecordingSpeakerPopoverAnchor }>()
  ...
  const [focusedSpeakerKey, setFocusedSpeakerKey] = useState<string>()
  ...
  useEffect(() => {
    setFocusedSpeakerKey(undefined)
  ...
  const focusedTranscript = recordingFocusedItems(transcriptItems, focusedSpeakerKey)
  ...
  const renderTranscript = () => {
    ...
    return <><ul style={styles.transcriptList}>{section.items.map(item => {
      const speakerFocused = focusedSpeakerKey === undefined || recordingTimelineSpeakerIdentity(item) === focusedSpeakerKey
      ...
      return <li key={item.itemId} style={{ ...styles.transcript, ...(selected ? { background: colors.layer1 } : {}), ...(!speakerFocused ? { opacity: .42 } : {}) }} onDoubleClick={() => { setSelectedTimelineMillis(item.startAtMillis) }}>
      <button type="button" style={styles.speakerAvatarTrigger} aria-label={`编辑说话人 ${item.speakerLabel}`} onClick={editSpeaker}>
      ...
      <span style={styles.transcriptBody}>
        <span style={styles.transcriptHeader}>
          <button type="button" style={styles.speakerNameTrigger} aria-label={`编辑说话人 ${item.speakerLabel}`} onClick={editSpeaker}><strong style={{ fontSize: 12, fontWeight: 500 }}>{item.speakerLabel}</strong></button>
    ...
    </li>})}</ul>{workbenchEnabled && editingSpeaker !== undefined && <ArkmeRecordingSpeakerEditor
