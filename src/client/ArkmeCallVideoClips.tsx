...
export function ArkmeCallVideoClips({ detail }: { detail: ArkmeCallDetail }) {
  ...
  return <>
    {clips.map((clip, index) => {
      ...
      const avatar = <ArkmeUserAvatar size={32} label={tr("{v0}的头像", { v0: participant?.displayName ?? clip.label ?? '通话参与者' })} {...(participant?.avatarRef ? { avatarRef: participant.avatarRef } : {})} />
      return <div key={`${clip.videoUrl}:${index}`} data-arkme-call-video={mine ? 'self' : 'peer'} style={{ display: 'flex', alignItems: 'flex-start', justifyContent: mine ? 'flex-end' : 'flex-start', gap: 8, margin: '4px 0 12px', flex: 'none' }}>
        {!mine && avatar}
        <button type="button" aria-label={tr("播放{v0}", { v0: clip.label || tr("通话录像") })} onClick={() => { setSelected(clip) }} style={{ width: 78, maxWidth: 'calc(100% - 40px)', flex: 'none', padding: '4px 4px 6px', border: 0, borderRadius: mine ? '16px 5px 16px 16px' : '5px 16px 16px 16px', background: mine ? arkmeTheme.messageOwn : arkmeTheme.messageOther, cursor: 'pointer' }}>
