...
export function ArkmeWorkspaceDialog({
  ...
  return <div className="arkme-workspace-dialog-backdrop" role="presentation" onMouseDown={event => {
    ...
    <section className="arkme-workspace-dialog" role="dialog" aria-modal="true" aria-labelledby="arkme-workspace-dialog-title">
      <header>
        <div><span>{tr("任务工作区")}</span><h2 id="arkme-workspace-dialog-title">{tr("选择 Agent 要处理的目录")}</h2></div>
        <button type="button" aria-label={tr("关闭工作区选择")} disabled={busy} onClick={onCancel}><X size={18} /></button>
      </header>
      <nav aria-label={tr("目录路径")}>
      ...
      <div className="arkme-workspace-directory-list">
        {loading && <div className="arkme-workspace-dialog-status">{tr("正在读取目录…")}</div>}
        ...
        {!loading && error === '' && visibleEntries.length === 0 && <div className="arkme-workspace-dialog-status">{tr("当前目录没有子目录")}</div>}
      </div>
      <footer>
        ...
        }}><ArrowLeft size={15} />{tr("上一级")}</button>
        <div><small>{listing?.path ?? tr("正在读取…")}</small><button type="button" disabled={loading || busy || listing === undefined} onClick={() => {
