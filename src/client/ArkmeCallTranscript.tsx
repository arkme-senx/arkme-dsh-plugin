...
export function callTranscriptClock(startMillis: number, startedAtMillis: number, spokenAtMillis?: number): string {
  ...
  return new Date(time).toLocaleTimeString(arkmeIntlLocale(), { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })
}

export function ArkmeCallTranscript({ detail }: { detail: ArkmeCallDetail }) {
  return <div aria-label={tr("通话转写")} style={{ display: 'flex', flexDirection: 'column', gap: 20, paddingTop: 18 }}>
    {detail.transcriptSegments.map(segment => {
      ...
      const avatar = <ArkmeUserAvatar size={32} label={tr("{v0}的头像", { v0: name })} {...(speaker?.avatarRef ? { avatarRef: speaker.avatarRef } : {})} />
    ...
    {detail.hangupParticipant && <div role="status" style={{ display: 'flex', alignItems: 'center', gap: 8, color: arkmeTheme.tertiary, fontSize: 12, marginTop: 4 }}>
      ...
      <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><PhoneDisconnectIcon size={14} aria-hidden />{detail.hangupParticipant.displayName}{tr("已挂断通话")}</span>
      <span aria-hidden style={{ flex: 1, borderTop: `1px solid ${arkmeTheme.borderSoft}` }} />
    </div>}
  </div>
}
