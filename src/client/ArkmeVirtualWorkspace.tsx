...
export function botActivityAtMillis(bot: ArkmeBotSummary): number {
  return Math.max(bot.createdAtMillis ?? 0, bot.latestActivityAtMillis ?? 0, bot.latestMessageAtMillis ?? 0)
}
...
export function ArkmeNavigation({
  ...
  const rootConversationRows = useMemo(() => [
    ...
    ...bots
      .filter(bot => botUsesPrivateConversationSurface(bot) && !botDirectoryIsHidden(botDirectoryPreferences, bot))
    ...
    >
      {directory === 'root' && <>
        ...
        {rootConversationRows.map(row => {
          ...
          >
            <span style={{ ...styles.chatRowRemoveContent, ...(removeFeedbackVisible ? styles.chatRowRemoveContentHidden : {}) }}>
              {sourceBot === undefined ? <span style={styles.sourceAvatarWrap}>
                  <ArkmeSourceAvatar
                    size={38}
