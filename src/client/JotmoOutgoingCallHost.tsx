...
import type { JotmoClientConfig, JotmoImagePayload } from '../types.js'
...
async function loadCallAvatar(imageRef: string): Promise<string> {
  const image = await callJotmo<JotmoImagePayload>('image.read', { imageRef })
  return `data:${image.mediaType};base64,${image.dataBase64}`
}

export function JotmoOutgoingCallHost() {
  ...
  useEffect(() => {
    ...
    void callJotmo<JotmoClientConfig>('auth.config')
