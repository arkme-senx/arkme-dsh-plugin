...
const phaseLabel: Record<RecordingImportSnapshot['phase'], string> = {
  prepared: '导入中', uploading: '导入中', finalizing: '导入中',
  accepted: '导入完成', failed: '导入失败', cancelled: '已取消',
}
...
export const ArkmeRecordingImportDialog = forwardRef<ArkmeRecordingImportDialogHandle, {
  ...
  const [jobDeletion, setJobDeletion] = useState<RecordingImportSnapshot>()
  ...
  const addFiles = (files: FileList | File[]) => {
    const fileNames = new Set(rows.map(row => row.file.name))
  ...
  return <>
    ...
    <dialog ref={dialogRef} style={styles.dialog} aria-label="上传文件" onCancel={event => {
      ...
      if (duplicateNames.length > 0) { skippedDuplicateNamesRef.current.clear(); setDuplicateNames([]); return }
      close()
    }} onClick={event => { if (event.target === event.currentTarget) close() }}>
      <header style={styles.header}><h2 style={styles.title}>上传文件</h2><button type="button" aria-label="关闭上传文件" style={styles.iconButton} onClick={close}><X size={18} aria-hidden /></button></header>
