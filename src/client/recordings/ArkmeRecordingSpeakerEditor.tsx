...
export function ArkmeRecordingSpeakerEditor({ item, anchor, onUpdated, onClose }: {
  ...
  const exactMatch = options.some(option => option.label.trim() === query.trim())

  useEffect(() => {
    const controller = new AbortController()
    setOptions([]); setSelected(''); setQuery(''); setBatch(false); setOptionsError(''); setMutationError(''); setLoading(true)
  ...
  const canBatch = item.sameSpeakerItemCount > 1 && item.speakerNumber >= 0
  ...
  const submitDisabled = !optionsReady || pending || selectedCurrent
  ...
  <div style={{ ...styles.popover, left: position.left, top: position.top }} role="dialog" aria-label="编辑说话人">
    ...
    {query.trim() !== '' && !exactMatch && <button type="button" aria-label="添加新说话人" style={styles.add} disabled={!optionsReady || pending} onClick={() => { void mutate() }}><Plus size={15} />添加“{query.trim()}”为说话人</button>}
    ...
    <div style={styles.bottom}>
      {canBatch ? <><input aria-label="批量修改" type="checkbox" checked={batch} disabled={pending} onChange={event => { setBatch(event.target.checked) }} /><span style={styles.batchText}>批量修改 {item.sameSpeakerItemCount} 处“{item.speakerLabel}”</span></> : <span style={styles.batchText}>仅修改当前片段</span>}
      <button type="button" style={{ ...styles.confirm, ...(submitDisabled ? { background: desktop.avatar, cursor: 'default' } : {}) }} disabled={submitDisabled} onClick={() => { void mutate() }}>{pending ? '保存中…' : '确认'}</button>
    </div>
  </div></>
