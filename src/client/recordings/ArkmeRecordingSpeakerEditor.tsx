...
export function ArkmeRecordingSpeakerEditor({ item, onUpdated, onClose }: {
  ...
  return <div style={styles.popover} role="dialog" aria-label="编辑说话人">
    ...
    {loading ? <span>正在读取候选…</span> : <select aria-label="选择说话人" value={selected} onChange={event => { setSelected(event.target.value) }}>
    ...
    {selected === '' && <input aria-label="新说话人名称" value={newName} maxLength={50} onChange={event => { setNewName(event.target.value) }} placeholder="输入说话人名称" />}
    {item.sameSpeakerItemCount > 1 && <label><input type="checkbox" checked={batch} onChange={event => { setBatch(event.target.checked) }} />批量修改当天 {item.sameSpeakerItemCount} 处“{item.speakerLabel}”</label>}
    {error !== '' && <div role="alert" style={styles.danger}>{error}</div>}
    <div style={styles.actions}>
      <button type="button" style={styles.button} disabled={pending || (selected === '' && newName.trim() === '')} onClick={() => { void mutate() }}>{batch ? `批量保存 ${item.sameSpeakerItemCount} 处` : '保存到当前片段'}</button>
