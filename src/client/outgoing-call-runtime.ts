...
type ApiCall = (operation: JotmoPluginOperation, params?: Record<string, unknown>) => Promise<unknown>
type BrowserPreparedCall = Omit<JotmoOutgoingCallPrepareResult['call'], 'calleeAvatar'> & { calleeAvatar: string }
type BrowserPrepared = Omit<JotmoOutgoingCallPrepareResult, 'call'> & { call: BrowserPreparedCall }
...
const FAILURE_CODES = new Set<JotmoOutgoingCallFailureCode>([
...
function failureFrom(error: unknown): { code: JotmoOutgoingCallFailureCode; message: string } {
  const body = error !== null && typeof error === 'object' && 'body' in error
    ? (error as { body?: unknown }).body : undefined
  const record = body !== null && typeof body === 'object' ? body as Record<string, unknown> : {}
  const code = typeof record.code === 'string' && FAILURE_CODES.has(record.code as JotmoOutgoingCallFailureCode)
    ? record.code as JotmoOutgoingCallFailureCode : 'call-bootstrap-failed'
...
export class OutgoingCallRuntime {
  ...
  private intent: JotmoOutgoingCallIntentClaim | undefined
  ...
  constructor(options: OutgoingCallRuntimeOptions = {}) {
    this.api = options.api ?? (callJotmo as ApiCall)
  ...
  async pollToolIntent(): Promise<void> {
    ...
    this.claiming = true
    try {
      const claim = await this.api('calls.outgoing.intent.claim') as JotmoOutgoingCallIntentClaim | null
  ...
  private async start(request: OutgoingCallUiRequest, intent?: JotmoOutgoingCallIntentClaim): Promise<void> {
    if (this.snapshot.phase !== 'idle') {
    ...
    try {
      ...
      }) as JotmoOutgoingCallPrepareResult
  ...
  private async fail(code: JotmoOutgoingCallFailureCode, message: string): Promise<void> {
    const intent = this.intent
    this.intent = undefined
    if (intent !== undefined) await this.api('calls.outgoing.intent.resolve', {
  ...
  private async finish(code: JotmoOutgoingCallFailureCode, message: string): Promise<void> {
    this.generation += 1
    const intent = this.intent
    this.intent = undefined
    if (intent !== undefined) await this.api('calls.outgoing.intent.resolve', {
