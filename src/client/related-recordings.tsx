...
import type {
  JotmoRelatedRecordingItem, JotmoRelatedRecordingMonthBucket, JotmoRelatedRecordingPageState,
} from '../types.js'
...
const colors = {
  ...
  accent: 'var(--dsw-alias-state-business-primary, #3964fe)',
  danger: '#c2413b',
}
const styles: Record<string, CSSProperties> = {
  panel: {
    position: 'absolute', zIndex: 30, top: 56, right: 0, bottom: 0, width: 'min(440px, 100%)',
    display: 'flex', flexDirection: 'column',
    borderLeft: `1px solid ${colors.border}`, background: colors.panel, boxSizing: 'border-box', boxShadow: '-8px 0 24px rgba(0,0,0,.04)',
  },
  ...
  modal: { width: 'min(720px, calc(100vw - 48px))', maxHeight: 'min(760px, calc(100vh - 48px))', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRadius: 18, background: colors.panel, boxShadow: '0 24px 80px rgba(0,0,0,.24)' },
  ...
  transcriptButton: { marginTop: 20, border: `1px solid ${colors.border}`, borderRadius: 10, padding: '8px 14px', background: colors.panel, color: colors.text, cursor: 'pointer' },
...
function dateValue(item: JotmoRelatedRecordingItem): number {
  return item.dateStamp ?? item.startAtMillis
}

export function groupRelatedRecordings(items: readonly JotmoRelatedRecordingItem[]): RelatedRecordingGroup[] {
...
export function mergeRelatedRecordingItems(
  current: readonly JotmoRelatedRecordingItem[],
  incoming: readonly JotmoRelatedRecordingItem[],
): JotmoRelatedRecordingItem[] {
  const seen = new Set<string>()
  const merged: JotmoRelatedRecordingItem[] = []
...
function participantText(item: JotmoRelatedRecordingItem): string {
...
export interface RelatedRecordingsPanelProps {
  contactName: string
  state: 'loading' | JotmoRelatedRecordingPageState
  stateMessage: string
  error: string
  items: JotmoRelatedRecordingItem[]
  hasMore: boolean
  ...
  onSelect(item: JotmoRelatedRecordingItem): void
}
...
export function RelatedRecordingDetail({ item, onClose }: { item: JotmoRelatedRecordingItem; onClose(): void }) {
  const [showTranscript, setShowTranscript] = useState(false)
  return <div style={styles.backdrop} role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}>
    <section style={styles.modal} role="dialog" aria-modal="true" aria-label="相关录音详情">
      <header style={styles.modalHeader}>
        <h2 style={styles.modalTitle}>{item.title || '未命名录音'}</h2>
        <button type="button" style={styles.iconButton} onClick={onClose} aria-label="关闭录音详情">×</button>
      </header>
      <div style={styles.modalBody}>
        <h3 style={styles.sectionLabel}>时段总结</h3>
        <p style={styles.detailSummary}>{item.summary || '暂无总结'}</p>
        <div style={styles.detailMeta}>{item.timeRangeText || '时间范围暂不可用'}<br />{participantText(item)}</div>
        {item.transcriptAvailable && item.transcript !== undefined && <Fragment>
          <button type="button" style={styles.transcriptButton} onClick={() => { setShowTranscript(value => !value) }}>
            {showTranscript ? '收起原文' : '查看原文'}
          </button>
          {showTranscript && <div style={styles.transcript}>{item.transcript}</div>}
        </Fragment>}
      </div>
    </section>
  </div>
