...
function TeamMessagingPanel({ accountKey, intent, onClose }: { accountKey: string; intent: TeamMessageIntent; onClose(): void }) {
  ...
  return <div className="team-message-backdrop" data-arkme-notification-blocking-overlay="true">
    <div ref={dialog} className="team-message-panel" data-team-detail-open={!!(selected || settings)} role="dialog" aria-modal="true" aria-label={tr("团队消息")} tabIndex={-1} onKeyDown={event => {
      ...
      <header className="team-panel-header">{(selected || settings) && <button className="team-narrow-back" onClick={() => { setSelected(undefined); setSettings(undefined) }}>{tr("返回会话列表")}</button>}<strong>{tr("团队消息")}</strong><span>{tr("团队共同接待，每位用户独立会话")}</span><button onClick={onClose} aria-label={tr("关闭团队消息")}>{tr("关闭")}</button></header>
      ...
      <div className="team-panel-layout">
        <aside className="team-inbox">
          <div className="team-tabs"><button aria-pressed={side === 'team'} onClick={() => { selectSide('team') }}>{tr("团队收件箱")}{(attention.team || attention.applications) && <span className="team-unread-dot" aria-label={tr("有未读消息")} />}</button><button aria-pressed={side === 'external'} onClick={() => { selectSide('external') }}>{tr("我的咨询")}{attention.external && <span className="team-unread-dot" aria-label={tr("有未读消息")} />}</button></div>
          <div className="team-list-actions"><button disabled={loading} onClick={() => { if (retryOpen.current) void retryOpen.current(); else void refresh() }}>{tr("刷新")}</button><select aria-label={tr("团队通道与成员管理")} value="" onChange={e => { if (e.target.value) { setSettings(e.target.value); setSelected(undefined) } }}><option value="">{tr("管理我的团队…")}</option>{teams.map(t => <option key={t.jotmoId} value={t.teamRef}>{t.name}</option>)}</select></div>
          {side === 'team' && <select aria-label={tr("筛选团队")} value={teamFilter} onChange={e => { filterRef.current = e.target.value; setTeamFilter(e.target.value); setSelected(undefined); setSettings(undefined); setPage({ items: [], hasMore: false }); pageRef.current = { items: [], hasMore: false }; void refresh() }}><option value="">{tr("全部团队")}</option>{teamFilter && !teams.some(t => t.teamRef === teamFilter) && <option value={teamFilter}>{tr("当前团队")}</option>}{teams.map(t => <option key={t.jotmoId} value={t.teamRef}>{t.name}</option>)}</select>}
          <nav aria-label={tr("团队会话")}>{page.items.map(c => <button key={c.key} className="team-conversation-row" aria-current={selected?.key === c.key} onClick={() => { setSelected(c); setSettings(undefined) }}>
