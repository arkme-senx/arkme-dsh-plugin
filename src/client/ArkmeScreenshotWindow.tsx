...
export function ArkmeScreenshotWindow() {
 const bridge=nativeScreenshotBridge(),[frame,setFrame]=useState<ScreenshotFrame>(),[error,setError]=useState(''),[windows,setWindows]=useState<Rect[]>([])
 useEffect(()=>{let disposed=false
  void bridge?.context().then(value=>{if(disposed)return;if(!value)throw new Error('截图会话已失效')
   setWindows(value.windows??[]);const bytes=decodeScreenshotBase64(value.contentBase64);setFrame({blob:new Blob([bytes],{type:'image/png'}),width:value.width,height:value.height})})
   .catch(e=>{if(!disposed){setError(String(e));void bridge?.close()}})
  return()=>{disposed=true}
 },[bridge])
 if(!bridge||!frame)return <div role="status">{error||'正在加载截图…'}</div>
 return <ArkmeScreenshotEditor frame={frame} desktop windows={windows} onClose={()=>{void bridge.close()}}
