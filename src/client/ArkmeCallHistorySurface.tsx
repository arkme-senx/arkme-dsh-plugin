...
export function ArkmeCallHistoryRow({ item, selected, playing: _playing, onSelect, onTogglePlayback }: {
  ...
  const detail = item.missed ? tr("未接通 · {v0}", { v0: mediaText }) : `${mediaText} · ${formatDuration(item.durationSeconds)}`
  return <li><div role="treeitem" aria-selected={selected} style={{ ...styles.item, ...(selected ? styles.itemActive : {}) }}>
    <button type="button" style={styles.itemSelect} aria-label={tr("查看与{v0}的{v1}记录", { v0: item.peerName, v1: mediaText })} onClick={onSelect} />
    ...
    {!item.missed ? <button type="button" style={styles.itemPlay} data-arkme-call-action={item.mediaType} aria-label={tr("向{v0}发起{v1}通话", { v0: item.peerName, v1: item.mediaType === 'video' ? tr("视频") : tr("语音") })} onClick={onTogglePlayback}><CallTypeIcon mediaType={item.mediaType} size={18} /></button> : <span style={styles.playSpacer} />}
  </div></li>
}
...
function DemoVideoReplay({ item, playing, currentTime, onToggle }: { item: ArkmeCallHistoryItem; playing: boolean; currentTime: number; onToggle(): void }) {
  ...
  return <section style={styles.videoPlayer} aria-label={tr("视频通话回放")} data-arkme-video-replay="demo">
    <div style={styles.videoLayoutBar}><button type="button" style={styles.videoLayoutTrigger} aria-expanded={menuOpen} onClick={() => { setMenuOpen(value => !value) }}>{tr("▦ 画面排版")}</button></div>
    {menuOpen && <div style={styles.videoLayoutMenu} aria-label={tr("视频回放画面排版")}>{([['peer', '对方视角'], ['self', '我的视角'], ['vertical', '上下排版'], ['horizontal', '左右排版']] as const).map(([value, label]) => <button key={value} type="button" style={{ ...styles.videoLayoutOption, ...(layout === value ? styles.videoLayoutOptionActive : {}) }} aria-pressed={layout === value} onClick={() => { setLayout(value); setMenuOpen(false) }}>{label}</button>)}</div>}
    <div style={{ ...styles.videoReplayPair, ...(layout === 'vertical' ? { gridTemplateColumns: '1fr', gap: 24 } : {}), ...((layout === 'peer' || layout === 'self') ? { gridTemplateColumns: '1fr', justifyItems: 'center' } : {}) }} data-arkme-video-perspectives={layout}>
      {showPeer && <div style={styles.videoReplayPerson}><DemoPersonAvatar item={item} style={styles.videoReplayAvatar} /><div style={styles.videoReplayCard}><button type="button" style={styles.videoReplayButton} aria-label={`${playing ? '暂停' : tr("播放")}${item.peerName}视角视频`} onClick={onToggle}><PlaybackIcon playing={playing} size={18} /></button></div></div>}
      {showSelf && <div style={{ ...styles.videoReplayPerson, ...styles.videoReplayPersonSelf }}><span style={{ ...styles.videoReplayAvatar, display: 'grid', placeItems: 'center', overflow: 'hidden', borderRadius: 999, background: arkmeTheme.layer2, fontWeight: 700 }} aria-hidden>{tr("我")}</span><div style={{ ...styles.videoReplayCard, ...styles.videoReplayCardSelf }}><button type="button" style={styles.videoReplayButton} aria-label={`${playing ? '暂停' : tr("播放")}我的视角视频`} onClick={onToggle}><PlaybackIcon playing={playing} size={18} /></button></div></div>}
    </div>
  </section>
}
...
function CallTypeSheet({ name, onSelect, onClose }: { name: string; onSelect(mediaType: ArkmeCallMediaType): void; onClose(): void }) {
  return <div style={styles.callSheetBackdrop} role="presentation" onClick={onClose}><section role="dialog" aria-modal="true" aria-label={tr("与{v0}通话", { v0: name })} style={styles.callSheet} onClick={event => { event.stopPropagation() }}><div style={styles.callSheetHandle} aria-hidden /><h3 style={styles.callSheetTitle}>{tr("与")}{name}{tr("通话")}</h3><p style={styles.callSheetHint}>{tr("选择通话方式")}</p><button type="button" style={styles.callSheetChoice} onClick={() => { onSelect('audio') }}><CallTypeIcon mediaType="audio" size={19} />{tr("语音通话")}</button><button type="button" style={styles.callSheetChoice} onClick={() => { onSelect('video') }}><CallTypeIcon mediaType="video" size={19} />{tr("视频通话")}</button><button type="button" style={styles.contactDialogClose} onClick={onClose}>{tr("取消")}</button></section></div>
}
...
export function ArkmeAddContactPage({ profile, contacts = [], assetBasePath = '/arkme-self/api/call', onBack, onSelectContact }: { profile: ArkmeUserProfile | null | undefined; contacts?: ArkmeCallContact[]; assetBasePath?: string; onBack(): void; onSelectContact?(contact: ArkmeCallContact): void }) {
  ...
  return <section style={styles.addContactPage} aria-label={tr("添加联系人页面")} data-arkme-add-contact-page="true">
    <header style={styles.addContactTop}><button type="button" style={styles.addContactBack} aria-label={tr("返回联系人与通话入口")} onClick={onBack}><img src={`${assetBasePath}/arrow_left.svg`} alt="" width="20" height="20" /></button><h3 style={styles.addContactTitle}>{tr("添加联系人")}</h3><span /></header>
    <form style={styles.addContactSearch} onSubmit={event => { event.preventDefault(); setStatus(searchValue.trim() === '' ? '请输入手机号或即我号' : `正在搜索“${searchValue.trim()}”`) }}><input style={styles.addContactInput} value={searchValue} aria-label={tr("输入手机号或即我号")} placeholder={tr("输入手机号或即我号")} onChange={event => { setSearchValue(event.target.value); setStatus('') }} /><button type="submit" style={styles.addContactSearchButton} aria-label={tr("搜索联系人")}><img src={`${assetBasePath}/image_search.svg`} alt="" width="23" height="23" /></button></form>
    <button type="button" style={styles.addContactScan} onClick={() => { setStatus('网页版扫码入口正在接入；可先输入即我号搜索') }}><img src={`${assetBasePath}/icon-scan-add-contact.svg`} alt="" width="24" height="24" /><span>{tr("扫一扫二维码添加好友")}</span><span aria-hidden style={{ color: c.tertiary, fontSize: 22 }}>›</span></button><p role="status" style={styles.addContactStatus}>{status}</p>
    {normalizedSearch !== '' && <ul style={styles.resultList} aria-label={tr("联系人搜索结果")}>{results.length > 0 ? results.map(contact => <li key={contact.sourceRef}><button type="button" style={styles.resultButton} onClick={() => { onSelectContact?.(contact) }}><RealContactAvatar contact={contact} /><span><strong style={styles.contactOptionTitle}>{contact.displayName}</strong><span style={styles.contactOptionCopy}>{tr("已在联系人中 · 点击选择通话方式")}</span></span><span aria-hidden>›</span></button></li>) : <li style={styles.empty}>{tr("没有在当前联系人中找到“")}{searchValue.trim()}”</li>}</ul>}
    <footer style={styles.addContactFooter}>{profile === undefined ? <p style={styles.addContactOwnerId}>{tr("正在读取当前账号资料…")}</p> : profile === null || arkmeId === '' ? <p style={styles.addContactOwnerId}>{tr("暂时无法读取当前账号资料")}</p> : <><div style={styles.addContactOwner}><p style={styles.addContactOwnerName}>{profile.displayName}</p><p style={styles.addContactOwnerId}>{tr("即我号：")}{arkmeId}</p></div><img src={profileQrDataUrl(arkmeId)} alt={tr("我的 Arkme 二维码")} style={styles.addContactQr} /></>}</footer>
  </section>
}
...
function TranscriptBubble({ item, line, active, playing, onPlay }: { item: ArkmeCallHistoryItem; line: ArkmeCallTranscriptLine; active: boolean; playing: boolean; onPlay(): void }) {
  ...
  return <li style={{ ...styles.messageRow, ...(self ? styles.messageRowSelf : {}) }} data-arkme-call-transcript-bubble={line.id}>
    ...
    <div style={{ ...styles.messageStack, ...(self ? styles.messageStackSelf : {}) }}><span style={styles.speakerName}>{line.speaker}</span>
      <button type="button" style={{ ...styles.bubble, ...(self ? styles.bubbleSelf : {}), ...(active ? styles.bubblePlaying : {}) }} aria-label={tr("播放{v0}在{v1}说的话", { v0: line.speaker, v1: spokenAt })} aria-pressed={active && playing} onClick={onPlay}>
...
function ParticipantCard({ item, participant, onClose }: { item: ArkmeCallHistoryItem; participant: 'self' | 'peer'; onClose(): void }) {
  ...
  return <div style={styles.participantCardBackdrop} role="presentation" onClick={onClose}>
    <section role="dialog" aria-modal="true" aria-label={`${self ? tr("我") : item.peerName}的通话参与人卡片`} style={styles.participantCard} onClick={event => { event.stopPropagation() }}>
      ...
      <h3 style={styles.participantCardName}>{self ? tr("我") : item.peerName}</h3><p style={styles.participantCardRole}>{profileOpen ? '个人主页' : tr("通话参与人")}</p>
      {profileOpen && <div style={styles.profilePanel}><strong>{self ? '我的 Arkme 主页' : tr("{v0}的 Arkme 主页", { v0: item.peerName })}</strong><p style={styles.profileFact}>{self ? '这里展示你的公开资料、世界动态和联系入口。' : `${item.peerRelationship} · 来自本次${item.mediaType === 'video' ? tr("视频") : tr("语音")}通话的参与人。`}</p><p style={styles.profileFact}>{tr("当前为通话演示版，正式接入账号资料后会打开完整主页。")}</p></div>}
      <button type="button" style={{ ...styles.participantProfileButton, marginTop: profileOpen ? 14 : 0 }} onClick={() => { setProfileOpen(value => !value) }}>{profileOpen ? '返回参与人卡片' : tr("查看个人主页")}</button>
    </section>
  </div>
}
...
export function ArkmeCallHistorySurface({ assetBasePath = '/arkme-self/api/call', contacts = [] }: { assetBasePath?: string; contacts?: ArkmeCallContact[] } = {}) {
  ...
  return <CallAssetBasePathContext.Provider value={assetBasePath}><div ref={rootRef} style={styles.root} data-arkme-call-history="prototype"><div style={{ ...styles.layout, gridTemplateColumns: compact ? '288px minmax(0,1fr)' : '326px minmax(0,1fr)' }}>
    <aside style={styles.listPane} aria-label={tr("通话记录列表")}><header style={styles.listHeader}>
      <div style={styles.listTitleLine}><h2 style={styles.listTitle}>{tr("通话")}</h2><span style={styles.demoChip}>{tr("第一版演示")}</span></div><p style={styles.listSubtitle}>{tr("找联系人，发起语音或视频通话；通话结束后可回放、看摘要和逐句转写。")}</p>
      <section style={styles.contactSection} aria-label={tr("联系人快捷入口")}><div style={styles.contactHeading}><h3 style={styles.contactTitle}>{tr("联系人")}</h3><span style={styles.contactHint}>{tr("选择联系人即可通话")}</span></div><div style={styles.contactRow}>
        <button type="button" style={styles.contactButton} aria-label={tr("打开联系人与通话入口")} onClick={() => { setContactDialogOpen(true) }}><span style={{ ...styles.contactAvatar, ...styles.contactAdd }}>＋</span><span style={styles.contactName}>{tr("添加")}</span></button>
        {contacts.slice(0, 3).map(contact => <button key={contact.sourceRef} type="button" style={styles.contactButton} aria-label={tr("选择联系人{v0}发起通话", { v0: contact.displayName })} onClick={() => { setCallFeedback(''); setCallTarget(contact) }}><RealContactAvatar contact={contact} /><span style={styles.contactName}>{contact.displayName}</span></button>)}
      </div></section>
      <div style={styles.searchWrap}><span style={styles.searchIcon} aria-hidden>⌕</span><input style={styles.search} value={query} aria-label={tr("搜索联系人")} placeholder={tr("搜索联系人")} onChange={event => { setQuery(event.target.value) }} /></div>
      <div style={styles.filters} aria-label={tr("通话类型筛选")}>{([['all', '全部'], ['audio', '语音'], ['video', '视频']] as const).map(([value, label]) => <button key={value} type="button" style={{ ...styles.filter, ...(mediaFilter === value ? styles.filterActive : {}) }} aria-pressed={mediaFilter === value} onClick={() => { setMediaFilter(value) }}>{label}</button>)}</div>
      {callFeedback !== '' && <p role="status" style={styles.addContactStatus}>{callFeedback}</p>}
    </header><ul style={styles.list} role="tree">{filteredItems.length > 0 ? filteredItems.map(item => <ArkmeCallHistoryRow key={item.id} item={item} selected={item.id === selected?.id && !detailDismissed} playing={item.id === selected?.id && playing} onSelect={() => { setDetailDismissed(false); setSelectedId(item.id) }} onTogglePlayback={() => { const realContact = contacts.find(contact => contact.displayName === item.peerName); setCallFeedback(''); setCallTarget(realContact ?? { sourceRef: '', displayName: item.peerName }) }} />) : <li style={styles.empty}>{tr("没有找到相关通话")}</li>}</ul></aside>

    <section style={styles.detail} aria-label={tr("通话详情")}><div ref={detailScrollRef} style={styles.detailScroll}><div style={styles.detailInner}>
      {selected === undefined || detailDismissed ? <div style={styles.empty}><strong>{tr("选择一条通话记录")}</strong><br />{tr("从左侧选择联系人，查看通话详情、回放和转写。")}</div> : (() => {
      ...
      {selected.missed ? <div style={styles.empty}><strong>{tr("这次")}{mediaText}{tr("没有接通")}</strong><br />{tr("接通并完成录制后，可以在这里回放、查看摘要和转写。")}</div> : <>
        ...
        <section style={{ ...styles.participantsSheet, ...(!participantsExpanded ? styles.participantsSheetCollapsed : {}) }} aria-label={tr("通话参与人")} data-arkme-call-participants="bottom-sheet" data-expanded={participantsExpanded ? 'true' : 'false'}>
          ...
          {participantsExpanded && <div id="arkme-call-participants-content" style={styles.participantList}>
            ...
            <button type="button" style={styles.participantChip} onClick={() => { setSelectedParticipant('self') }}><img src={selfAvatarUrl} alt="" style={styles.participantChipAvatar} aria-hidden="true" /><span>{tr("我")}</span></button>
          </div>}
        </section>
      </>}
      </>
